import {
  ...
  Dialog,
  Option,
} from "@/game-engine/Dialog.js";
import type { Location } from "@/game-engine/Location.js";
...
export class Game<T extends IGameServerAPI, M> {
  ...
  public async loadMap(mapFilePath: string): Promise<void> {
    ...
    for (const [index, tileSet] of Object.entries(map.tileSets)) {
      const texture = await Assets.load(tileSet.content);
  ...
  public updateViewport() {
    if (this.man) {
      this.app.stage.x = 0.5 * this.app.canvas.width - this.man.x;
      this.app.stage.y = 0.5 * this.app.canvas.height - this.man.y;
    }
  }
...
function createImage(content: string): Promise<HTMLImageElement> {
